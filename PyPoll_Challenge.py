...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:
        ...
        vote_percentage_c = float(votes_c) / float(total_votes_c) * 100
        county_results = (
            f"{county_name}: {vote_percentage_c:.1f}% ({votes_c:,})\n")
        # 6d: Print the county results to the terminal.
        print(county_results)
        # 6e: Save the county votes to a text file.
        txt_file.write(county_results)
        # 7: Print the county with the largest turnout to the terminal.
        if (votes_c > largest_turnout_count) and (vote_percentage_c > largest_turnout_percentage):
            largest_turnout_count = votes_c
